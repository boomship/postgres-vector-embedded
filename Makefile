...
ifeq ($(PLATFORM),darwin)
    ...
    OPENSSL_PREFIX := /c/ProgramData/chocolatey/lib/openssl.light/local
    CONFIGURE_FLAGS = --prefix=$(PREFIX) --disable-nls --with-openssl --without-icu --without-llvm --without-lz4 --without-zstd --without-libxml --with-includes="$(OPENSSL_PREFIX)/include" --with-libraries="$(OPENSSL_PREFIX)/lib"
else
